...
public class MyAppointmentsFragment extends Fragment {
    ...
    private void init() {
        ...
        if(mMyAppointmentsDataModel!=null) {
           ...
           /* List<ClinicList> mClinicLists = new ArrayList<>();
            List<PatientList> mPatientLists = new ArrayList<>();
            for(int i = 0;i<mMyAppointmentsDataModel.getClinicList().size();i++){
                ClinicList clinicList = new ClinicList();
                for(int j= 0;i< mMyAppointmentsDataModel.getClinicList().get(i).getPatientList().size();j++){
                    if(j!=0){
                        PatientList patientList = new PatientList();
                        patientList = mMyAppointmentsDataModel.getClinicList().get(i).getPatientList().get(j);
                        mPatientLists.add(patientList);
                        clinicList.setPatientList(mPatientLists);
                    }
            ...
            mAppointmentAdapter = new AppointmentAdapter(getActivity(), mMyAppointmentsDataModel.getClinicList());
